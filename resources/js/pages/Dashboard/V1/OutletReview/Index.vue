...
const selectedItem = ref<OutletReviewItem | null>(null);
...
const hasActiveFilters = computed(() => {
    return !!(searchQuery.value || ratingFilter.value !== 'all' || outletFilter.value !== 'all' || activeFilter.value !== 'all' || replyFilter.value !== 'all');
});

const applyFilters = (overrides: { page?: number; per_page?: number } = {}) => {
    router.get('/dashboard/outlet-reviews', {
...
const handleFilterChange = (type: string) => (value: string | number | boolean | bigint | Record<string, unknown> | null | undefined) => {
    ...
    if (type === 'outlet') outletFilter.value = String(value || 'all');
...
const handleClearFilters = () => {
    ...
    router.get('/dashboard/outlet-reviews', { per_page: perPage.value }, { preserveState: true });
};

const handleView = (review: { id: number }) => router.visit(`/dashboard/outlet-reviews/${review.id}`);
const handleEdit = (review: { id: number }) => router.visit(`/dashboard/outlet-reviews/${review.id}/edit`);
const handleReply = (review: { id: number }) => router.visit(`/dashboard/outlet-reviews/${review.id}/reply`);
const handleDelete = () => {
    if (!selectedItem.value) return;
    isDeleting.value = true;
    router.delete(`/dashboard/outlet-reviews/${selectedItem.value.id}`, {
        onSuccess: () => {
...
<template>
    <Head :title="__('Outlet Reviews')" />

    <div class="flex h-full flex-1 flex-col gap-6 p-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">{{ __('Outlet Reviews') }}</h1>
                <p class="text-muted-foreground">{{ __('Manage customer outlet/store reviews') }}</p>
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-4">
            ...
            <Select :model-value="outletFilter" @update:model-value="handleFilterChange('outlet')">
                <SelectTrigger class="w-[200px]"><SelectValue :placeholder="__('Outlet')" /></SelectTrigger>
                <SelectContent>
                    <SelectItem value="all">{{ __('All Outlets') }}</SelectItem>
                    <SelectItem v-for="outlet in outlets" :key="outlet.id" :value="String(outlet.id)">{{ outlet.name }}</SelectItem>
                </SelectContent>
            </Select>
